...
// Returns an environment variable that should be excluded from the browser.
// Shhh! It's a secret!
function secret(key) {
  var p = process;
  return p.env[key];
...
module.exports = rc('sdcportal', {
  ...
  auth: {
    provider: 'admin-party',
    session: {
      secret: secret('SDCPORTAL_SESSION_SECRET') || 'shhh'
    }
  },
  client: {
    portal: {
      ...
      actual: {
        rootUrl: 'http://' + hostname + ':' + port + '/api'
      }
    }
  },
  sdc: {
    // Please provide via ~/.sdcportalrc or /etc/sdcportal/config
  }
});
