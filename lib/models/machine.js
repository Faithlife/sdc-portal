...
Machine.prototype.loadExtendedData = function loadExtendedData(client) {
  ...
  return when.settle([
    client.getMachineAudit(self._machine.id),
    client.getImage(self._machine.image)
  ])
    .then(function (promiseResults) {
      if (promiseResults[0].state === 'fulfilled') {
        self._audit = promiseResults[0].value;
      }

      if (promiseResults[1].state === 'fulfilled') {
        self._image = promiseResults[1].value;
      }
