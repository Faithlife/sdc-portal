    ...
    authProvider.getUsers(request)
      .then(function (users) {
        ...
        ws.on('close', function (event) {
          logger.log('client socket closed, removing 1 socket. curr length: %d', poller.sockets.length);
          poller.sockets.splice(poller.sockets.indexOf(ws), 1);

          var pollerSocketLength = poller.sockets.length;
          logger.log('socket removed, new length: %d', pollerSocketLength);

          if (pollerSocketLength === 0) {
            clearInterval(poller.timerId);
            delete pollers[poller.key];
          }
        });
      });
  };
